...
var _chunkAHDAEK3Dcjs = require('./chunk-AHDAEK3D.cjs');


var _chunkXD2JABEDcjs = require('./chunk-XD2JABED.cjs');
...
var FileHelper = class _FileHelper {
    ...
    try {
      ...
      switch (url.protocol) {
        ...
        case "onedrive:": {
          ...
          driveId = _chunkAHDAEK3Dcjs.OneDriveService.formatDriveId(driveId);
          itemId = _chunkAHDAEK3Dcjs.OneDriveService.formatItemId(itemId);
          return {
            type: "onedrive",
        ...
        case "sharepoint:": {
          ...
          const driveId = _chunkAHDAEK3Dcjs.OneDriveService.formatDriveId(_nullishCoalesce(pathParts[0], () => ( "")));
          const itemId = _chunkAHDAEK3Dcjs.OneDriveService.formatItemId(_nullishCoalesce(pathParts[1], () => ( "")));
          return {
            type: "sharepoint",
        ...
        case "http:":
        case "https:": {
          if (_chunkXD2JABEDcjs.CloudStorageService.isGCSUrl(fileUrl)) {
            const info = _chunkXD2JABEDcjs.CloudStorageService.extractInfo(fileUrl);
            if (!info) {
              throw new (0, _verror.VError)(`Invalid GCS URL format, expecting 'gs://{bucketName}/{filePath}'`);
            }
            return {
              type: "gcs",
              uri: _chunkXD2JABEDcjs.CloudStorageService.toGCSUri(info),
  ...
  static encodeUri(fileUri) {
    switch (fileUri.type) {
      ...
      case "onedrive":
        return `onedrive://${_chunkAHDAEK3Dcjs.OneDriveService.formatDriveId(fileUri.driveId)}/${_chunkAHDAEK3Dcjs.OneDriveService.formatItemId(fileUri.itemId)}`;
      case "sharepoint":
        return `sharepoint://${fileUri.siteId}/${_chunkAHDAEK3Dcjs.OneDriveService.formatDriveId(fileUri.driveId)}/${_chunkAHDAEK3Dcjs.OneDriveService.formatItemId(fileUri.itemId)}`;
      case "public":
        return fileUri.uri;
      default:
...
//# sourceMappingURL=chunk-MDTVL2BK.cjs.map