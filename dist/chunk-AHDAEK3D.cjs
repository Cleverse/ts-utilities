...
var OneDriveService = class _OneDriveService {
  ...
  static extractDriveItemFromURL(sharePointUrl) {
    ...
    if (drivesIndex === -1 || itemsIndex === -1 || drivesIndex >= itemsIndex) {
      throw new (0, _chunk6KSVIB3Gcjs.BadRequestError)(`OneDriveService: Invalid SharePoint URL format: ${sharePointUrl}`);
    }
    const driveId = pathSegments[drivesIndex + 1];
    const itemId = pathSegments[itemsIndex + 1];
    if (!driveId || !itemId) {
      throw new (0, _chunk6KSVIB3Gcjs.BadRequestError)(`OneDriveService: Could not extract driveId or itemId from URL: ${sharePointUrl}`);
    }
    return {
...
//# sourceMappingURL=chunk-AHDAEK3D.cjs.map