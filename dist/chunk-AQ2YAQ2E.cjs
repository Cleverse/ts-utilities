...
var EnvelopeEncryptionService = class {
  ...
  async #getDEK(version) {
    return _chunkCU6GGSDDcjs.SingleFlight.withLock("ts-utilities:encryption:getdek", async () => {
      ...
      const encryptedDek = await this.sharedDEKDatagateway.getDEK(_nullishCoalesce(version, () => ( this.config.defaultVersion)));
      if (!encryptedDek) {
        return null;
      }
...
//# sourceMappingURL=chunk-AQ2YAQ2E.cjs.map