...
var _chunkDDH27FXXcjs = require('./chunk-DDH27FXX.cjs');
...
var CloudStorageService = class _CloudStorageService {
  ...
  async getFileMetadata(fileUrl) {
    try {
      ...
      throw new (0, _chunkDDH27FXXcjs.SomethingWentWrong)(`Failed to get file info: ${fileUrl}`, {
        cause: error
      });
    }
  }
    ...
    if (info) {
      ...
      try {
        ...
        if (isGCSError(error)) {
          if (error.code === 404 || error instanceof Error && _optionalChain([error, 'access', _6 => _6.message, 'optionalAccess', _7 => _7.toLowerCase, 'call', _8 => _8(), 'access', _9 => _9.includes, 'call', _10 => _10("not found")])) {
            throw new (0, _chunkDDH27FXXcjs.NotFoundError)(`File not found in storage: gs://${info.bucketName}/${info.filePath}`, {
              cause: error
            });
          }
          if (error.code === 403) {
            throw new (0, _chunkDDH27FXXcjs.ForbiddenError)(`Access denied to file: gs://${info.bucketName}/${info.filePath}`, {
              cause: error
            });
          }
        }
        throw new (0, _chunkDDH27FXXcjs.SomethingWentWrong)(`Failed to download from GCS: gs://${info.bucketName}/${info.filePath}`, {
          cause: error
        });
      }
    } else {
      ...
      if (!response.ok) {
        if (response.status === 404) {
          throw new (0, _chunkDDH27FXXcjs.NotFoundError)(`File not found: ${fileUrl}`);
        }
        if (response.status >= 400 && response.status < 500) {
          throw new (0, _chunkDDH27FXXcjs.BadRequestError)(
            `Failed to fetch file from ${fileUrl}: Got status code ${response.status} ${response.statusText}`
          );
        }
        throw new (0, _chunkDDH27FXXcjs.SomethingWentWrong)(
          `Failed to fetch file from ${fileUrl}: Got status code ${response.status} ${response.statusText}`
        );
      }
  ...
  async upload(bucketName, filePath, content, options) {
    ...
    try {
      ...
      throw new (0, _chunkDDH27FXXcjs.SomethingWentWrong)(`Failed to upload file to gs://${bucketName}/${filePath}`, {
        cause: error
      });
    }
    const [metadata] = await file.getMetadata();
  ...
  async uploadFromUrl(bucketName, filePath, fileDownloadUrl, options) {
    ...
    if (!response.ok) {
      if (response.status === 404) {
        throw new (0, _chunkDDH27FXXcjs.NotFoundError)(`File not found: ${fileDownloadUrl}`);
      }
      if (response.status >= 400 && response.status < 500) {
        throw new (0, _chunkDDH27FXXcjs.BadRequestError)(
    ...
    if (!stream) {
      throw new (0, _chunkDDH27FXXcjs.SomethingWentWrong)(`Failed to get stream from ${fileDownloadUrl}`, {
    ...
    try {
      ...
      if (isGCSError(error)) {
        if (error.code === 404 || error instanceof Error && _optionalChain([error, 'access', _15 => _15.message, 'optionalAccess', _16 => _16.toLowerCase, 'call', _17 => _17(), 'access', _18 => _18.includes, 'call', _19 => _19("not found")])) {
          throw new (0, _chunkDDH27FXXcjs.NotFoundError)(`File not found for signed URL generation: gs://${info.bucketName}/${info.filePath}`);
        }
        if (error.code === 403) {
          throw new (0, _chunkDDH27FXXcjs.ForbiddenError)(`Access denied to generate signed URL for: gs://${info.bucketName}/${info.filePath}`);
        }
      }
      throw new (0, _chunkDDH27FXXcjs.SomethingWentWrong)(`Failed to generate signed URL for file: ${fileUrl}`, {
        cause: error
      });
    }
  }
  async checkResourceType(resourceUrl) {
    ...
    try {
      ...
      throw new (0, _chunkDDH27FXXcjs.SomethingWentWrong)(`Failed to check item type: ${resourceUrl}`, {
        cause: error
      });
    }
  }
...
//# sourceMappingURL=chunk-SZPGJFJA.cjs.map